...
function pataa_display_order_data_in_admin( $order ){ ?>
    <div class="order_data_column">
        <h4><?php _e( 'Pataa Detail' ); ?></h4>
        <?php 
		$api_key = get_option("settings_page_example_setting");
		$order = new WC_Order($order->get_id()); // Order id		
		$billing_address = $order->get_address('billing'); 
		if (strpos($billing_address['address_1'], '^') !== false) {
			$add_array = explode(",", $billing_address['address_1']);
			$pataa_code = strtoupper(ltrim($add_array[0], '^'));
			echo '<img src="https://pataa.com/cpimages/logo-input.svg" alt="">';
		}
		$data = array(
			'api_key' => $api_key,
			'pc' => $pataa_code
		...
		$crl = curl_init('https://pataa.in:5057/get-api-plan');
		...
		if ($result === false) {
			...
			if(isset($obj->result->share_link) && $obj->result->share_link!="")
			{
			echo '&nbsp;<a href='.$obj->result->share_link.' target="_blank">'.$pataa_code.'</a>';
			}
		}
